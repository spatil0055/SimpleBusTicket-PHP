...
(1, 'Shreyas', 'admin', '$2y$10$7rLSvRVyTQORapkDOqmkhetjF6H9lJHngr4hJMSM2lHObJbW5EQh6', '2021-06-02 13:55:21'),
(2, 'Somnath', 'testadmin', '$2y$10$A2eGOu1K1TSBqMwjrEJZg.lgy.FmCUPl/l5ugcYOXv4qKWkFEwcqS', '2021-10-17 21:10:07');
...
FOR EACH ROW
BEGIN
    DECLARE booked_seat INT;
    DECLARE bus_no_var VARCHAR(155);
    
    -- Get the booked seat and bus number
    SELECT booked_seat, route_id INTO booked_seat, bus_no_var FROM bookings WHERE id = NEW.id;
    
    -- Update the seats table to mark the booked seat as occupied
    UPDATE seats SET seat_booked = CONCAT_WS(',', COALESCE(seat_booked, ''), booked_seat) WHERE bus_no = bus_no_var;
END$$
DELIMITER ;

DELIMITER $$
CREATE TRIGGER after_route_assignment
...
BEGIN
    ...
    -- Get the bus number
    SELECT bus_no INTO bus_no_var FROM routes WHERE id = NEW.id;
    
    -- Update the buses table to mark the bus as assigned
    UPDATE buses SET bus_assigned = 1 WHERE bus_no = bus_no_var;
END$$
DELIMITER ;

CREATE VIEW booking_details AS
SELECT b.booking_id, c.customer_name, r.route_cities, r.route_dep_date, r.route_dep_time, b.booked_amount, b.booked_seat, b.booking_created
FROM bookings b
